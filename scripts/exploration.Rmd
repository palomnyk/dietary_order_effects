...
# Super pathways
urine_sup_pv <- pathway_loop(result_pv = urine_pvals,
                             pathway_col = "SUPER_PATHWAY",
                             pathway_df = urine_metabo_path,
                             metabolomic_df = urine_metabolomics)
plasma_sup_pv <- pathway_loop(result_pv = plasma_pvals,
                             pathway_col = "SUPER_PATHWAY",
                             pathway_df = plasma_metabo_path,
                             metabolomic_df = plasma_metabolomics)   
    
# Sub pathways
urine_sub_pv <- pathway_loop(result_pv = urine_pvals,
                             pathway_col = "SUB_PATHWAY",
                             pathway_df = urine_metabo_path,
                             metabolomic_df = urine_metabolomics)
plasma_sub_pv <- pathway_loop(result_pv = plasma_pvals,
                             pathway_col = "SUB_PATHWAY",
                             pathway_df = plasma_metabo_path,
                             metabolomic_df = plasma_metabolomics)   
...
my_df <- data.frame("pathways" = c(names(plasma_sup_pv), names(urine_sup_pv)), 
                    "pval" = -log10(c(plasma_sup_pv, urine_sup_pv)),
                    "tissue" = c(rep("plasma", length(plasma_sup_pv)), rep("urine", length(urine_sup_pv))))
...
ggplot(my_df, aes(x = pathways, y = pval, color = factor(tissue))) +
  ...
  ggtitle(paste0("ADL Super pathway analysis")) + # Blank Title for the Graph
          ...
          theme(line = element_line(colour = "black", linewidth = 1), 
                ...
                # axis.title.x = element_text(family=font_type,colour = "Black", margin = margin(t = 20, r = 0, b = 0, l =0)),
                # axis.title.y = element_text(family=font_type,colour = "Black", margin = margin(t = 0, r = 20, b = 0, l = 0)),
                # plot.title = element_text(family=font_type, colour = "Black", margin = margin(t = 0, r = 0, b = 20, l = 0)),
                # axis.text=element_text(family=font_type,size=axis_font_size, color = "Black"),
                # text=element_text(family=font_type,size=axis_font_size), plot.margin = margin(t = 2, r = 2, b = 2, l = 2, unit = "cm")
          )
```
...
my_df <- data.frame("pathways" = c(names(plasma_sub_pv), names(urine_sub_pv)), 
                    "pval" = -log10(c(plasma_sub_pv, urine_sub_pv)),
                    "tissue" = c(rep("plasma", length(plasma_sub_pv)), rep("urine", length(urine_sub_pv))))
...
#### Read metadata ####
lclf_metadata <- openxlsx::read.xlsx(xlsxFile = file.path("data", "SampleLabIDs.xlsx"),
                                     sheet = 1)
print(paste("original lclf_metadata nrow/ncol:", paste(dim(lclf_metadata), collapse = "/")))

lclf_samp_link <- openxlsx::read.xlsx(xlsxFile = file.path("data", "NCIA-0106-20PHML+", "NCIA-0106-20PHML+ DATA TABLES (PLASMA)_CURRENT.xlsx"),
                                     sheet = "Sample Meta Data")
print(paste("original lclf_samp_link nrow/ncol:", paste(dim(lclf_samp_link), collapse = "/")))
lclf_metadata <- na.omit(lclf_metadata)

print(paste("na lclf_metadata nrow/ncol:", paste(dim(lclf_metadata), collapse = "/")))

lclf_metadata$PARENT_SAMPLE_NAME <- lclf_samp_link$PARENT_SAMPLE_NAME[match(lclf_metadata$CLIENT_SAMPLE_ID, lclf_samp_link$CLIENT_SAMPLE_ID)]

lclf_metadata <- na.omit(lclf_metadata, cols = "PARENT_SAMPLE_NAME")

row.names(lclf_metadata) <- lclf_metadata$PARENT_SAMPLE_NAME

lclf_metadata$SUBJECT_ID <- as.factor(lclf_metadata$CLIENT_SAMPLE_ID)
lclf_metadata$DIET <- as.factor(lclf_metadata$Diet)
lclf_metadata$DIET_ORDER <- as.factor(lclf_metadata$ARM)

lclf_metadata <- lclf_metadata[, c("SUBJECT_ID", "DIET", "DIET_ORDER")]

print(paste("final lclf_metadata nrow/ncol:", paste(dim(lclf_metadata), collapse = "/")))
```


```{r Load and organize lclf metabolomics}
#### Read lclf data ####
lclf_plasma_mtbmcs <- openxlsx::read.xlsx(xlsxFile = file.path("data", "NCIA-0106-20PHML+", "NCIA-0106-20PHML+ DATA TABLES (PLASMA)_CURRENT.xlsx"),
                                     sheet = "Batch-normalized Data", rowNames = TRUE)
print(paste("original lclf_plasma_mtbmcs nrow/ncol:", paste(dim(lclf_plasma_mtbmcs), collapse = "/")))

lclf_chem_link <- openxlsx::read.xlsx(xlsxFile = file.path("data", "NCIA-0106-20PHML+", "NCIA-0106-20PHML+ DATA TABLES (PLASMA)_CURRENT.xlsx"),
...
# row.names(lclf_plasma_mtbmcs) <-  lclf_samp_link$PARENT_SAMPLE_NAME[match()]

lclf_plasma_mtbmcs <- merge(lclf_metadata, lclf_plasma_mtbmcs, by = 0)
...
initial_table <- lclf_plasma_mtbmcs[,5:ncol(lclf_plasma_mtbmcs)]
