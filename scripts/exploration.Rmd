...
initial_table <- data.frame(data.table::fread(file = metabolon_file,
                                   data.table = FALSE))
...
my_df$pathways[which(my_df$pval > -log10(0.05))] <- toupper(my_df$pathways[which(my_df$pval > -log10(0.05))])
...
my_df$super[my_df$super == "Xenobiotics"] <- "Xenobio"
...
g <- ggplot(my_df, aes(x = pathways, y = pval, color = factor(tissue))) +
  ...
  geom_vline(xintercept = vert_lines, color = "blue", alpha=0.4)
